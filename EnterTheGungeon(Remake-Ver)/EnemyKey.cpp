...
void EnemyKey::init(FPOINT scale, const POINT& sp, POINT size, POINT currPos, POINT currIdx)
{
	Monster::init();
	_scale = move(scale);
	_size = move(size);
...
void EnemyKey::update()
{
	Monster::update();
	_weapon.update();
}

void EnemyKey::render(const HDC& hdc) const
{
	Monster::render(hdc);
	_weapon.render(hdc);
}

void EnemyKey::release()
{
	Monster::release();
}

void EnemyKey::changeIdleMotion(const MONSTER_STANDING_POSITION_STATE& selectedStandingState, bool isReverseX)
{
}

void EnemyKey::changeWalkingMotion(const MONSTER_STANDING_POSITION_STATE& selectedStandingState, bool isReverseX)
{
}

void EnemyKey::changeDeathMotion(const MONSTER_STANDING_POSITION_STATE& selectedStandingState, bool isReverseX)
{
}
